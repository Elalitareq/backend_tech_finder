...
export async function getAllTechnicians(req, res) {
    try {
      ...
      const technicians = await TechnicianModel.find({service});
  
      const modifiedTechnicians = [];
...
export const updateTechnician = async (req, res) => {
  try {
    const { id } = req.params;
    const technicianData = req.body;
    const technician = await TechnicianModel.findByIdAndUpdate(id, technicianData, { new: true });
    if (technician) {
      res.json(technician);
    } else {
      res.status(404).json({ error: 'Technician not found' });
